...
$sql = sprintf("select uid,register_time,register_platform,substring(from_unixtime(register_time),12,2),substring(from_unixtime(register_time),15,2),DAYNAME(from_unixtime(register_time)) from user where register_platform=2 and substring(from_unixtime(register_time),1,10)>='2012-02-13'");
#print $sql . "\n";
$dbh = $dbh_sanguo;
$sth = $dbh->prepare($sql);
