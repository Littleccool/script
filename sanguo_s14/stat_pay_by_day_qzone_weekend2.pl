...
$sql=sprintf("select uid from user_basic where vip_level>0" );
#print $sql . "\n";
$dbh = $dbh_sanguo;
$sth = $dbh->prepare($sql);
...
$sql = sprintf("select uid,register_time,register_platform,substring(from_unixtime(register_time),12,2),substring(from_unixtime(register_time),15,2),DAYNAME(from_unixtime(register_time)) from user where register_platform=4 and substring(from_unixtime(register_time),1,10)>='2012-02-13'");
#print $sql . "\n";
$dbh = $dbh_sanguo;
$sth = $dbh->prepare($sql);
if($DBI::errstr){print $sql . "\n";}
...
while (@row = $sth->fetchrow_array)
{
	...
	$dayname{$uId} = $row[5];
	#if($dayname{$uId} eq "Saturday" || $dayname{$uId} eq "Sunday" || $dayname{$uId} eq "Friday")
	#{
	#}
	#else
	#{
	#	next;
	#}
	$register_hour{$uId} = $dayname{$uId} . "." . $register_hour{$uId};
	$WeekDayByUid{$uId} = $dayname{$uId};
	...
	$UserCountByWeekDay{$dayname{$uId}}++;
	$UidIsCounted{$uId} = 1;
	if($IsVipUser{$uId})
	{
		$VipUserByWeekDay{$dayname{$uId}}++;
	}
}
$sth->finish();
...
while (@row = $sth->fetchrow_array)
{
	...
	$PayAmountByWeekDay{$WeekDayByUid{$uId}} += $credit;
	if(!$IsPayNumberCounted{$uId})
	{
		#printf("%d\n",$uId);
		$PayNumbersByWeekDay{$WeekDayByUid{$uId}}++;
		$IsPayNumberCounted{$uId} = 1;
	}
...
printf("tmp:%d\n",$PayNumbersByWeekDay{"Sunday"});
printf("tmp2:%d\n",$VipUserByWeekDay{"Sunday"});
...
foreach $key ( sort {$PayAmountByWeekDay{$b}/$UserCountByWeekDay{$b} <=> $PayAmountByWeekDay{$a}/$UserCountByWeekDay{$a} } keys(%UserCountByWeekDay))
{
	printf("%s:user count:%d,\tpay sum:%.2frmb,\taverage:%.3frmb,\tpay user per:%.2f%%,\tvip user per:%.2f%%\n",$key,$UserCountByWeekDay{$key},$PayAmountByWeekDay{$key}/100,$PayAmountByWeekDay{$key}/$UserCountByWeekDay{$key},100*$PayNumbersByWeekDay{$key}/$UserCountByWeekDay{$key},100*$VipUserByWeekDay{$key}/$UserCountByWeekDay{$key});
}
