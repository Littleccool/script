...
print "\n";
print '<html><head><title>qq login stat</title>
...
$cmd = "/usr/local/script/sanguo_s14/monitor_login/self_log.pl";
system($cmd);

$dbh = &setDBStat();

printf("<h1 align=center>qq login stat</h1>");
...
$sql = sprintf("select date,all_rookie_count,d1,d2,d3,d4,d5,d6,d7,d14,d30 from table_other_return where cp=\'sl\' order by date desc limit 30");
$sth = $dbh->prepare($sql);
