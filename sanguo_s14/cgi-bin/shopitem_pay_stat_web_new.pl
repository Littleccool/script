...
for($i=0;$i<31;$i++)
{
	...
	$date =~ tr/\r\n//d;
        #print "date====$date\n";
	my $file = sprintf("%s/rmbshopitem_new_%s.log",$statdir,$date);
		...
		if($line)
		{
			...
			if($line_list_num == 5)
			{
				my @item_list = split(/\,/,$line_list[3]);
                                ...
                                {
                                    @myitem = split(/_/,$item_list[$j]);
                                    $item_list[$j] = $myitem[2];
                                }
...
{
	my $temp_stat = $all_stat{$key}{"cost_stat1"};
...
$mfile = sprintf("/usr/local/script/sanguo_s14/cgi-bin/shop_item.txt");
unless(open(MYFILE, $mfile))
{
        #print 'can not open file ' . $mfile . "!\n";
        #exit -1;
}
while ($line=<MYFILE>)
{
...
foreach $e (@items)
{
       ...
       @nb = split(/_/,$e);
       if($IsKeyValue{$nb[0]}){ 
         print "<td colspan='2'>$IsKeyValue{$nb[0]}</td>";
       }
       else{
         print "<td colspan='2'>SHOP_",$e,"</td>";
       }
}

print "</tr>\r\n";
print "<tr><td>G</td><td>Z</td>";
foreach $e (@items)
{
      print "<td>G</td><td>Z</td>";
}
print "</tr>\r\n";
$s=0;
