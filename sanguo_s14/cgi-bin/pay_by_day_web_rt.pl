...
$today = `date +%Y%m%d`;
$today =~ tr/\r\n//d;
`/usr/local/script/sanguo_s14/stat_pay_by_day_all.pl $today`;
...
{
	my $before = $i - 1;
	my $date = `date -d "$before days ago" +%Y%m%d`;
	$date =~ tr/\r\n//d;
...
print "<tr><td>date</td><td>new users</td>";
for($i = 1; $i <= 30; $i++)
{
	print "<td colspan=2>d$i</td>";
}
$userTotal = 0;
$creditTotal = 0;
print "<td colspan=2>total</td><td>date</td></tr>\r\n";
foreach my $key (sort {$b<=>$a} keys(%allStatMap)){
	...
	print "<tr><td id='d$key' style='display:block'>$key</td><td id='dd$key' style='display:none'><font color='red'>$key</font></td><td>$newUsers,$newvipamt($viprate)</td>";
...
print "<tr><td>total</td><td>$allNewUsers</td>";
for($i = 1; $i <= 30; $i++)
{
	if ($allUsersByDay{$i}){
