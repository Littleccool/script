...
$sql = sprintf("select level,uid from user where unix_timestamp($Date)-register_time>3600*24*2 and unix_timestamp($Date)-register_time<=3600*24*3");
$dbh = $dbh_sg;
$sth = $dbh->prepare($sql);
...
#print "$Date\n";
$myFile = sprintf(">%s/data/rmbuserpay/Compare3_%s.log",$monitor_path,$Date);
...
$test=0;
$myFile3=sprintf(">%s/data/rmbuserpay/Compare3_%s.log",$monitor_path,$Date0);
